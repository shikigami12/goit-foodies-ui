...
export const CategoryList = ({ categories }) => {
  ...
  return (
    <ul className="grid gap-4 md:grid-cols-2 md:gap-5 xl:grid-cols-15">
      {categories.map(({ id, name, thumb }, index) => {
        ...
        return (
          <li key={id} className={clsx("rounded-[20px] overflow-hidden relative md:rounded-[30px] md:max-lg:nth-[5n+3]:col-span-2", {
            ...
            <div className="absolute inset-0 p-4 flex items-end gap-1 bg-black/20">
              ...
              <button className="flex items-center justify-center p-2.5 border border-white/20 rounded-full text-white md:p-3 hover:border-white transition-colors cursor-pointer active:border-white/60 active:text-white/60">
